...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        pad_len = max(0,max_length - len(sentence))
        sentence = sentence[0:max_length] + [zero_vector]*pad_len
        labels = labels[0:max_length] + [zero_label]*pad_len
        mask = [True]*(max_length - pad_len) + [False]*pad_len
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable(name = 'b2',shape = (self.config.n_classes),initializer = tf.contrib.layers.xavier_initializer())
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                out,h = cell(x[:,time_step,:],h)
                out_drop = tf.nn.dropout(out,dropout_rate)
                pred_t = tf.matmul(h,U) + b2 # here the shape of the vector is (batch_size,n_classes)
                preds.append(pred_t)               
                ### END YOUR CODE

        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
        preds = tf.stack(preds,axis = 1)  #same as the following two which are commented
       # preds = tf.stack(preds,axis = 0) #here the shape of the tensor is (max_length,batch_size,n_classes)
       # preds = tf.transpose(preds,perm = [1,0,2])#transpose it to a tensor with shape of (batch_size,max_length,n_classes) so we can compute the CE loss
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(pred_mask,label_mask)
